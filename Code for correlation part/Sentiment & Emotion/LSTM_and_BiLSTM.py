...
tokenizer.fit_on_texts(list(X_train_new) + list(X_val_new))

sequences_train = tokenizer.texts_to_sequences(X_train_new)
sequences_val = tokenizer.texts_to_sequences(X_val_new)
sequences_test = tokenizer.texts_to_sequences(X_test_new)
...
with tf.device('/device:GPU:0'):
    ...
    df_report.to_csv('../PSIMILAN/lstm_sentiments_report.csv')

with open('../PSIMILAN/Probs/word2vec_LSTM_GoEmotion_sentiment_probs.pickle', 'wb') as prob:
    pickle.dump(probs, prob)

model.save('../PSIMILAN/model/word2vec_LSTM_GoEmotion_sentiment.h5')
...
with tf.device('/device:GPU:0'):
    ...
    report = classification_report(y_test, labels_pred, target_names=labels_name, output_dict=True)
    df_report = pd.DataFrame(report).transpose()
    df_report.to_csv('../PSIMILAN/bilstm_sentiments_report.csv')

with open('../PSIMILAN/Probs/word2vec_BiLSTM_GoEmotion_sentiment_probs.pickle', 'wb') as prob:
    pickle.dump(probs, prob)

model1.save('../PSIMILAN/model/word2vec_BiLSTM_GoEmotion_sentiment.h5')
