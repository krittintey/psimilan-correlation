...
def initialize_model(epochs=10):
    ...
    bert_classifier = BertForSequenceClassification.from_pretrained('bert-base-multilingual-uncased', num_labels=4, output_attentions=False, output_hidden_states=False)
...
def train(model, train_dataloader, val_dataloader=None, epochs=10, evaluation=False):
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            torch.save(model.state_dict(), f'../PSIMILAN/models/Fine_Tuning_BERT_GoEmotion_Sentiment_epoch{epoch_i+1}.h5')
        print("\n")
...
with open('../PSIMILAN/Probs/Fine_Tuning_BERT_GoEmotion_Sentiment_probs.pickle', 'wb') as prob:
...
df_report.to_csv('../PSIMILAN/bert_sentiments_report.csv')
...
df_report = pd.DataFrame(report).transpose()
df_report.to_csv('../PSIMILAN/bert_sentiments_sample_validation_report.csv')
