...
def initialize_model(epochs=10):
    ...
    bert_classifier = BertForSequenceClassification.from_pretrained('setu4993/LaBSE', num_labels=num_labels, output_attentions=False, output_hidden_states=False)
...
def train(model, train_dataloader, val_dataloader=None, epochs=10, evaluation=False):
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            torch.save(model.state_dict(), f'../PSIMILAN/models/Fine_Tuning_LaBSE_Suicide_epoch{epoch_i+1}.h5')
        print("\n")
...
with open('../PSIMILAN/Probs/Fine_Tuning_LaBSE_Suicide_probs.pickle', 'wb') as prob:
...
df_report.to_csv('../PSIMILAN/labse_suicide_report.csv')
...
df_report = pd.DataFrame(report).transpose()
df_report.to_csv('../PSIMILAN/labse_suicide_sample_validation_report.csv')
